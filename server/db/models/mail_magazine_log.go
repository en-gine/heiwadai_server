...
func (o *MailMagazineLog) GetMailMagazine() *MailMagazine {
	if o == nil {
		return nil
	}

	return o.R.GetMailMagazine()
}
...
func (o *MailMagazineLog) GetUser() *UserDatum {
	if o == nil {
		return nil
	}
...
func (mailMagazineLogL) LoadMailMagazine(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMailMagazineLog interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args[object.MailMagazineID] = struct{}{}
		...
		for _, obj := range slice {
			...
			args[obj.MailMagazineID] = struct{}{}
	...
	query := NewQuery(
		...
		qm.WhereIn(`mail_magazine.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *MailMagazineLog) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(mailMagazineLogAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(mailMagazineLogPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert mail_magazine_log, could not build conflict column list")
			}

			conflict = make([]string, len(mailMagazineLogPrimaryKeyColumns))
			copy(conflict, mailMagazineLogPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"mail_magazine_log\"", updateOnConflict, ret, update, conflict, insert, opts...)

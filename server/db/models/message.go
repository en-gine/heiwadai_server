...
func (o *Message) GetAuthor() *Admin {
	if o == nil {
		return nil
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(messagePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert message, could not build conflict column list")
			}

			conflict = make([]string, len(messagePrimaryKeyColumns))
			copy(conflict, messagePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"message\"", updateOnConflict, ret, update, conflict, insert, opts...)

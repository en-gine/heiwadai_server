...
func (o *Store) GetStayableStoreInfo() *StayableStoreInfo {
	if o == nil {
		return nil
	}

	return o.R.GetStayableStoreInfo()
}
...
func (o *Store) GetBelongToAdmins() AdminSlice {
	if o == nil {
		return nil
	}

	return o.R.GetBelongToAdmins()
}
...
func (o *Store) GetBookPlans() BookPlanSlice {
	if o == nil {
		return nil
	}

	return o.R.GetBookPlans()
}
...
func (o *Store) GetCheckins() CheckinSlice {
	if o == nil {
		return nil
	}

	return o.R.GetCheckins()
}
...
func (o *Store) GetCoupons() CouponSlice {
	if o == nil {
		return nil
	}

	return o.R.GetCoupons()
}

func (r *storeR) GetCoupons() CouponSlice {
	if r == nil {
		return nil
	}
...
func (o *Store) Coupons(mods ...qm.QueryMod) couponQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"coupon_stores\" on \"coupon\".\"id\" = \"coupon_stores\".\"coupon_id\""),
...
func (storeL) LoadStayableStoreInfo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`stayable_store_info.store_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (storeL) LoadBelongToAdmins(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`admin`),
		qm.WhereIn(`admin.belong_to in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (storeL) LoadBookPlans(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`book_plan`),
		qm.WhereIn(`book_plan.store_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (storeL) LoadCheckins(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`checkin`),
		qm.WhereIn(`checkin.store_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (storeL) LoadCoupons(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	var slice []*Store
	var object *Store
	...
	for results.Next() {
		...
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice coupon")
		}
...
func (o *Store) AddCoupons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Coupon) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"coupon_stores\" (\"store_id\", \"coupon_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
// Sets o.R.Stores's Coupons accordingly.
// Replaces o.R.Coupons with related.
// Sets related.R.Stores's Coupons accordingly.
func (o *Store) SetCoupons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Coupon) error {
	query := "delete from \"coupon_stores\" where \"store_id\" = $1"
	...
	removeCouponsFromStoresSlice(o, related)
	if o.R != nil {
		o.R.Coupons = nil
	}
...
func (o *Store) RemoveCoupons(ctx context.Context, exec boil.ContextExecutor, related ...*Coupon) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"coupon_stores\" where \"store_id\" = $1 and \"coupon_id\" in (%s)",
...
func removeCouponsFromStoresSlice(o *Store, related []*Coupon) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(storePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert store, could not build conflict column list")
			}

			conflict = make([]string, len(storePrimaryKeyColumns))
			copy(conflict, storePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"store\"", updateOnConflict, ret, update, conflict, insert, opts...)

...
func (o *BookPlan) GetStore() *Store {
	if o == nil {
		return nil
	}
...
func (o *BookPlan) GetPlanBookPlanStayDateInfos() BookPlanStayDateInfoSlice {
	if o == nil {
		return nil
	}

	return o.R.GetPlanBookPlanStayDateInfos()
}
...
func (o *BookPlan) GetUserBooks() UserBookSlice {
	if o == nil {
		return nil
	}
...
func (bookPlanL) LoadPlanBookPlanStayDateInfos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBookPlan interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`book_plan_stay_date_info.plan_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (bookPlanL) LoadUserBooks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBookPlan interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_book`),
		qm.WhereIn(`user_book.book_plan_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *BookPlan) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(bookPlanAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(bookPlanPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert book_plan, could not build conflict column list")
			}

			conflict = make([]string, len(bookPlanPrimaryKeyColumns))
			copy(conflict, bookPlanPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"book_plan\"", updateOnConflict, ret, update, conflict, insert, opts...)

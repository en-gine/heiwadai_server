...
var UserWhere = struct {
	...
	FirstNameKana: whereHelperstring{field: "\"user\".\"first_name_kana\""},
	LastNameKana:  whereHelperstring{field: "\"user\".\"last_name_kana\""},
	CompanyName:   whereHelpernull_String{field: "\"user\".\"company_name\""},
	BirthDate:     whereHelpertime_Time{field: "\"user\".\"birth_date\""},
	ZipCode:       whereHelpernull_String{field: "\"user\".\"zip_code\""},
	Prefecture:    whereHelperint{field: "\"user\".\"prefecture\""},
	City:          whereHelpernull_String{field: "\"user\".\"city\""},
	Address:       whereHelpernull_String{field: "\"user\".\"address\""},
	Tel:           whereHelpernull_String{field: "\"user\".\"tel\""},
	Mail:          whereHelperstring{field: "\"user\".\"mail\""},
	AcceptMail:    whereHelperbool{field: "\"user\".\"accept_mail\""},
	CreateAt:      whereHelpertime_Time{field: "\"user\".\"create_at\""},
	UpdateAt:      whereHelpertime_Time{field: "\"user\".\"update_at\""},
}
...
func (r *userR) GetCheckins() CheckinSlice {
	if r == nil {
		return nil
	}
	return r.Checkins
...
var (
	userAllColumns            = []string{"id", "first_name", "last_name", "first_name_kana", "last_name_kana", "company_name", "birth_date", "zip_code", "prefecture", "city", "address", "tel", "mail", "accept_mail", "create_at", "update_at"}
	userColumnsWithoutDefault = []string{"id", "first_name", "last_name", "first_name_kana", "last_name_kana", "birth_date", "prefecture", "mail", "accept_mail"}
	userColumnsWithDefault    = []string{"company_name", "zip_code", "city", "address", "tel", "create_at", "update_at"}
...
func (o *User) UserOptions(mods ...qm.QueryMod) userOptionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"user_option\".\"user_id\"=?", o.ID),
	)
...
// LoadUserOptions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserOptions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_option")
	}

	var resultSlice []*UserOption
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_option")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_option")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.UserOptions = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.UserID) {
				local.R.UserOptions = append(local.R.UserOptions, foreign)
				if foreign.R == nil {
					foreign.R = &userOptionR{}
				}
				foreign.R.User = local
...
// Sets o.R.User's Checkins accordingly.
// Replaces o.R.Checkins with related.
// Sets related.R.User's Checkins accordingly.
func (o *User) SetCheckins(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Checkin) error {
...
// Sets related.R.User.
func (o *User) RemoveCheckins(ctx context.Context, exec boil.ContextExecutor, related ...*Checkin) error {
	if len(related) == 0 {
		return nil
	}
...
// AddUserOptions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserOptions.
// Sets related.R.User appropriately.
func (o *User) AddUserOptions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserOption) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.UserID, o.ID)
	...
	if o.R == nil {
		...
		o.R.UserOptions = append(o.R.UserOptions, related...)
	}
...
// SetUserOptions removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's UserOptions accordingly.
// Replaces o.R.UserOptions with related.
// Sets related.R.User's UserOptions accordingly.
func (o *User) SetUserOptions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserOption) error {
	query := "update \"user_option\" set \"user_id\" = null where \"user_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.UserOptions {
			queries.SetScanner(&rel.UserID, nil)
			if rel.R == nil {
				continue
			}
	...
	return o.AddUserOptions(ctx, exec, insert, related...)
}

// RemoveUserOptions relationships from objects passed in.
// Removes related items from R.UserOptions (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveUserOptions(ctx context.Context, exec boil.ContextExecutor, related ...*UserOption) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.UserOptions {
			if rel != ri {
				continue
			}

			ln := len(o.R.UserOptions)
			if ln > 1 && i < ln-1 {
				o.R.UserOptions[i] = o.R.UserOptions[ln-1]
			}
			o.R.UserOptions = o.R.UserOptions[:ln-1]
			break
		}
	}

...
func testUserBooksInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(userBookPrimaryKeyColumns, userBookColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	userBookDBTypes = map[string]string{`ID`: `uuid`, `TLBookingNumber`: `character varying`, `StayFrom`: `timestamp with time zone`, `StayTo`: `timestamp with time zone`, `Adult`: `integer`, `Child`: `integer`, `RoomCount`: `integer`, `CheckInTime`: `character varying`, `TotalCost`: `integer`, `GuestDataID`: `uuid`, `BookPlanID`: `uuid`, `BookUserID`: `uuid`, `Note`: `text`, `CreateAt`: `timestamp with time zone`, `UpdateAt`: `timestamp with time zone`, `TLBookdataID`: `character varying`, `DelateAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)

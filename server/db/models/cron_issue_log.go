...
func (o *CronIssueLog) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(cronIssueLogAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(cronIssueLogPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert cron_issue_log, could not build conflict column list")
			}

			conflict = make([]string, len(cronIssueLogPrimaryKeyColumns))
			copy(conflict, cronIssueLogPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"cron_issue_log\"", updateOnConflict, ret, update, conflict, insert, opts...)

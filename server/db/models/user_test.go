...
func testUserToManyCheckins(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCheckins(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyUserOptions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userOptionDBTypes, false, userOptionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userOptionDBTypes, false, userOptionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserOptions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserOptions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserOptions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserOptions = nil
	if err = a.L.LoadUserOptions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserOptions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpCheckins(t *testing.T) {
	var err error
...
func testUserToManySetOpCheckins(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpCheckins(t *testing.T) {
	var err error
...
func testUserToManyAddOpUserOptions(t *testing.T) {
	...
	var a User
	var b, c, d, e UserOption

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserOption{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userOptionDBTypes, false, strmangle.SetComplement(userOptionPrimaryKeyColumns, userOptionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserOption{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserOptions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserOptions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserOptions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserOptions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpUserOptions(t *testing.T) {
	var err error
	...
	err = a.SetUserOptions(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetUserOptions(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.UserOptions[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.UserOptions[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpUserOptions(t *testing.T) {
	var err error
	...
	err = a.AddUserOptions(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveUserOptions(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.UserOptions) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.UserOptions[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.UserOptions[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}
